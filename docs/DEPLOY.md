...
It must print `2.2.16` on **every** host. If one box is behind, push the same tag again. Link libraries from Settings after both are on the same version. AirPlay binaries land in `/data/opt/airplay`.
