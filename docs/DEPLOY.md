...
It must print `2.2.30`. AirPlay binaries land in `/data/opt/airplay`. NAS tools land in `/data/opt/nas` (`rclone` is not in git; `push-host.sh` copies it from `host-webui/nas/`). Time Clock UI is removed.
